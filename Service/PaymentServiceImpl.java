...
/*	private Payment addDetailsPayment(Payment p) {
		 ...
		 * while (status=False)
		 * {
		 * if (Method=="Yearly") {InstallmentsNb=1; RemainingPremium=TTCPremium-RefundAmount;}
		 * else if {Method=="Monthly") {InstallmentsNb=12; RemainingPremium=(TTCPremium-(PaidPremium+RefundAmount));}
		 * status= True
		 * }
		 * */
